			...
			<div class="panel panel-default">
				<div class="panel-body">
					<div class="row vdivided"
						...
						<h3 style="display: inline-block; padding-left: 20px;">Contribuição
							no Projeto ${repository.name}</h3>
						<button style="margin-top: 15px"
							class="btn btn-lg btn-primary pull-right" data-toggle="modal"
							data-target="#testInfo-config">
						...
						<div class="modal fade" id="testInfo-config" role="dialog">
							<div class="modal-dialog modal-lg">
								<div class="modal-content">
									...
									<form
										action="${linkTo[RepositoryController].testConfig(repository.id)}"
										method="post">
															...
															<div class="row" style="height: 74px;">
																...
																<div class="col-lg-4" style="width: 210px;">
																	<div class="form-group">
																		<label for="timeWindow"><fmt:message
																				key="repository.config.time_window" />:</label> <select
																...
																<div class="col-lg-4" style="width: 180px;">
																	<div class="form-group date">
																		...
																		<label for="initDate"><fmt:message
																				key="repository.config.date.init" />:</label> <input
																			type="text" class="form-control datepicker"
																			id="initDate" placeholder="Data inicial"
																...
																<div class="col-lg-4" style="width: 180px;">
																	<div class="form-group date">
																		...
																		<label for="endDate"><fmt:message
																				key="repository.config.date.end" />:</label> <input
																			type="text" class="form-control datepicker"
																			id="endDate" placeholder="Data final"
										...
										<div style="padding-left: 20px; padding-bottom: 20px;">
											<button type="button" id="addTest" class="btn btn-primary"
												style="left: 10px">Definir como Teste</button>
										</div>
		...
		$(document).ready(function() {
							...
							function show() {
								if ($('#select1 option:selected').val() == 'Todos') {
									...
									chartsPlot($('#select1 option:selected')
											.val());
								}
														...
														.chart(
																'chart',
																{
																	colors : [
																			...
																			'#FF0000' ],
																	title : {
																		text : 'Histórico de Commits do Projeto '
																				+ repositoryName,
																		x : -20
																	//center
																	},
												...
												Highcharts
														.chart(
																'chart',
																{
																	...
																	title : {
																		text : 'Histórico de Commits do Desenvolvedor '
																				+ nome,
																		x : -20
																	//center
																	},
										...
										success : function(data) {
											...
											for (i = 0; i < data.length; i++) {
												...
												if (data[i].absolute > 0) {
													text += "<tr class='info'><td>"
															+ k
															+ "</td><td>"
															+ data[i].name
															+ "</td><td>"
															+ Number(
																	((data[i].absolute * 100) / (data[i].y + data[i].absolute)))
																	.toFixed(2)
															+ "%</td><td>"
															+ data[i].y
															+ "</td><td>"
											...
											text += "<tr class='success'><td>-</td><td>Total</td><td>"
													+ Number(
															((sumTest * 100) / (sum + sumTest)))
															.toFixed(2)
													+ "%</td><td>"
													+ sum
													+ "</td><td>"
